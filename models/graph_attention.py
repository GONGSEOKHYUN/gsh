...
from models.graph_mm_naive import graph_mm_naive as graph_mm_tvm
import argparse
import time
import numpy as np
...
def get_k_q(q_k_mask):
    ...
    for i in range(len(q_k_mask)):
        for j in range(len(q_k_mask[0])):
            if q_k_mask[i, j] >= 0:
                try:
                    index = torch.where(q_k_mask[q_k_mask[i, j]] == i)[0]
                    if len(index) > 0:
                        k_q_mask[i, j] = index[0]
                    else:
                        k_q_mask[i, j] = -1
                except:
                    k_q_mask[i, j] = -1    
...
class GraphSelfAttention(nn.Module):
    def __init__(self, opt):
        ...
        self.w_qs = nn.Linear(opt.d_k, opt.d_k, bias=False)
        self.w_ks = nn.Linear(opt.d_k, opt.d_k, bias=False)
        self.w_vs = nn.Linear(opt.d_k, opt.d_k, bias=False)
        ...
        self.layer_norm = nn.LayerNorm(opt.d_k, eps=1e-6)
    ...
    #     bsz, seq_len, n_head, d_k = hidden_states.size()
    ...
    def forward(self, hidden_states):
        bsz, seq_len, n_head, d_k = hidden_states.size()
        residual = hidden_states.reshape(bsz, seq_len, n_head * d_k).contiguous()
        ...
        q = q.view(bsz, seq_len, self.n_head, self.d_k).contiguous()
        k = k.view(bsz, seq_len, self.n_head, self.d_k).contiguous()
        v = v.view(bsz, seq_len, self.n_head, self.d_k).contiguous()
